...
* **Các bước làm**:
    ...
    4. Thiết lập đặc trưng mới **`Study_Productive_Ratio`** (Tỷ lệ ưu tiên học tập):
       $$\text{Study Productive Ratio} = \frac{\text{Study Hours Per Day}}{\text{Total Productive Hours}}$$
...
*   **Các bước làm**:
    1. Tổ chức mã nguồn chuẩn OOP bằng cách tạo thư mục `src/classification/` với các module riêng biệt (`models.py`, `optimizer.py`, `evaluate.py`, `train.py`).
    ...
    3. **Tối ưu hóa Siêu tham số (Hyperparameter Tuning)**: Ứng dụng thư viện **Optuna** tiên tiến để tự động tìm kiếm bộ tham số tốt nhất (best params) cho 2 mô hình phức tạp là RandomForestClassifier và SVC thông qua 5-Fold Cross Validation.
    4. **Phân tích Overfitting**: Sử dụng **Learning Curve** với 5-Fold Cross Validation để trực quan hóa hiệu suất của tập Train và tập Validation theo từng kích thước dữ liệu đối với toàn bộ 5 mô hình. Từ đó phân tích mô hình nào bị "học vẹt" (overfitting) hoặc học kém (underfitting).
    5. Đánh giá chất lượng mô hình bằng các chỉ số: `Accuracy`, `Precision`, `Recall`, `F1-Score` (tất cả tính theo trung bình `weighted` do phân loại 3 lớp).
    6. Trực quan hóa lỗi: Vẽ ma trận nhầm lẫn chuẩn hóa (Normalized Confusion Matrix) bằng `ConfusionMatrixDisplay` để xem mô hình thường phân loại nhầm lớp nào nhiều nhất. Lưu tất cả ảnh vào `outputs/figures/`.
    7. Lưu model xịn nhất vào thư mục `models/` (bằng joblib/pickle) và xuất bảng so sánh chỉ số của 5 mô hình ra file `outputs/classification_results.csv`.
